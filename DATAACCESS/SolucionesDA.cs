        ...
        public int Insertar(BE.SolucionesBE objDatosBE)
        {
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    ...
                    cmd.CommandText = "SP_SOLUCIONES_INSERTAR";
                    cmd.Parameters.Add("pClave", MySqlDbType.VarChar, 255).Value = objDatosBE.clave;
                    ...
                    if (intResultado > 0)
                        ...
                        throw new Exception("La solución ya fue registrada previamente");
                    else
        ...
        public int Modificar(BE.SolucionesBE objDatosBE)
        {
            try
            {
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    int intResultado;
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.CommandText = "SP_SOLUCIONES_MODIFICAR";
                    cmd.Parameters.Add("pClave", MySqlDbType.VarChar, 255).Value = objDatosBE.clave;
                    cmd.Parameters.Add("pDescripcion", MySqlDbType.VarChar, 5000).Value = objDatosBE.descripcion;
                    cmd.Parameters.Add("pIdUsuario", MySqlDbType.Int32).Value = objDatosBE.id_usuario;
                    cmd.Parameters.Add("pFecha", MySqlDbType.Datetime).Value = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
        ...
        public int Eliminar(BE.SolucionesBE objDatosBE)
        {
            try
            {
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    int intResultado;
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.CommandText = "SP_SOLUCIONES_ELIMINAR";
                    cmd.Parameters.Add("pId", MySqlDbType.Int32).Value = objDatosBE.id;
                    cmd.Parameters.Add("pIdUsuario", MySqlDbType.Int32).Value = objDatosBE.id_usuario;
                    cmd.Parameters.Add("pFecha", MySqlDbType.Datetime).Value = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
                    cmd.Parameters["pId"].Direction = System.Data.ParameterDirection.InputOutput;
